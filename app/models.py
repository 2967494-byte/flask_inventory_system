...
class User(UserMixin, db.Model):
    ...
    favorited_products = db.relationship(
        "Product", secondary=user_favorites, lazy="dynamic", backref="favorited_by"
    )
    ...
    def average_rating(self):
        ...
        avg_rating = (
            ...
            .filter(Review.seller_id == self.id, Review.is_published == True)
            .scalar()
        )
    ...
    def reviews_count(self):
        ...
        return Review.query.filter(
            Review.seller_id == self.id, Review.is_published == True
        ).count()
...
class Review(db.Model):
    ...
    seller = db.relationship(
        "User", foreign_keys=[seller_id], backref="reviews_received"
    )
...
class Message(db.Model):
    ...
    # Опциональная привязка к товару (если сообщение касается конкретного товара)
    product_id = db.Column(db.Integer, db.ForeignKey("product.id"), nullable=True)
    ...
    product = db.relationship("Product", foreign_keys=[product_id], backref="messages")

    def __repr__(self):
        return f"<Message {self.id} от {self.sender.email} к {self.recipient.email}>"
    ...
    def delete_for_sender(self):
        """Удалить сообщение для отправителя"""
        self.is_deleted_by_sender = True
        if self.is_deleted_by_recipient:
            # Если оба пользователя удалили сообщение, удаляем его полностью
            db.session.delete(self)
        db.session.commit()

    def delete_for_recipient(self):
        """Удалить сообщение для получателя"""
    ...
    def get_conversation(cls, user1_id, user2_id, limit=50):
        ...
        return (
            ...
            .filter(
                db.or_(
                    db.and_(
                        cls.sender_id == user1_id, cls.is_deleted_by_sender == False
                    ),
                    db.and_(
