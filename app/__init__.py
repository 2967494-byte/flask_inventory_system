...
def create_app():
    ...
    if telegram_bot:
        ...
        print(
            f"[OK] Telegram бот инициализирован: токен={'установлен' if app.config.get('TELEGRAM_BOT_TOKEN') else 'не указан'}, chat_id={app.config.get('TELEGRAM_CHAT_ID')}"
        )
    else:
    ...
    admin_flask.add_view(
        ModelView(Region, db.session, name="Регионы", category="Справочники")
    )
