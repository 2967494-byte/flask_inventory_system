...
from app.models import City, Product, Region
...
def get_dadata_company():
    try:
        ...
        print(
            f"[DEBUG] DaData INN Search: {inn}, Key: {'***' + api_key[-4:] if api_key and len(api_key) > 4 else 'INVALID'}"
        )
        ...
        try:
            ...
            print("[ERROR] DaData API Timeout")
            return jsonify(
                {
                    "error": "Сервис временно недоступен. Попробуйте позже или заполните данные вручную."
                }
            ), 503
        except requests.exceptions.ConnectionError:
            print("[ERROR] DaData API Connection Error")
            return jsonify(
                {
                    "error": "Не удалось подключиться к сервису. Проверьте интернет-соединение или заполните данные вручную."
                }
            ), 503
        except requests.exceptions.RequestException as e:
            print(f"[ERROR] DaData API Request Error: {e}")
            return jsonify(
                {"error": "Ошибка при запросе к сервису. Заполните данные вручную."}
            ), 503
        ...
        if response.status_code != 200:
            ...
            if response.status_code == 401:
                ...
                return jsonify(
                    {"error": "Доступ к API запрещен (баланс или тариф)"}
                ), 500
            return jsonify(
                {"error": f"Ошибка сервиса данных: {response.status_code}"}
            ), 500
        ...
        traceback.print_exc()
        return jsonify({"error": "Произошла ошибка. Заполните данные вручную."}), 500
...
def get_locations():
    try:
        ...
        if not search:
            ...
            regions_start = (
                Region.query.filter(Region.name.ilike(f"{search_lower}%"))
            ...
            all_regions = regions_start + [
                r for r in regions_contains if r not in regions_start
            ]
            ...
            cities_start = (
                City.query.join(Region)
                .filter(City.name.ilike(f"{search_lower}%"))
                .order_by(City.name)
                .limit(limit // 2)
                .all()
            )
            ...
            all_cities = cities_start + [
                c for c in cities_contains if c not in cities_start
            ]
...
def get_regions():
    ...
    for region in regions:
        region_data = {"id": region.id, "name": region.name, "cities": []}
